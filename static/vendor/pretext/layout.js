  ...
  for (let mi = 0; mi < analysis.len; mi++) {
    ...
    if (segKind === "text" && segMetrics.containsCJK) {
      ...
      for (const gs of graphemeSegmenter.segment(segText)) {
        ...
        const w = getCorrectedSegmentWidth(
          unitText,
          unitMetrics,
          emojiCorrection,
        );
    ...
    const lineEndFitAdvance =
      segKind === "space" ||
      segKind === "preserved-space" ||
      segKind === "zero-width-break"
        ? 0
        : w;
    const lineEndPaintAdvance =
      segKind === "space" || segKind === "zero-width-break" ? 0 : w;
    if (segWordLike && segText.length > 1) {
      const graphemeWidths = getSegmentGraphemeWidths(
        segText,
        segMetrics,
        cache,
        emojiCorrection,
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const startSegmentIndex =
      chunk.startSegmentIndex < preparedStartByAnalysisIndex.length
        ...
        : (preparedEndByAnalysisIndex[preparedEndByAnalysisIndex.length - 1] ??
          0);
...
function buildLineTextFromRange(
  ...
  for (let i = startSegmentIndex; i < endSegmentIndex; i++) {
    ...
    if (i === startSegmentIndex && startGraphemeIndex > 0) {
      text += getSegmentGraphemes(i, segments, cache)
        .slice(startGraphemeIndex)
        .join("");
    } else {
      text += segments[i];
    }
...
export function layoutWithLines(prepared, maxWidth, lineHeight) {
  ...
  const lineCount = walkPreparedLines(
