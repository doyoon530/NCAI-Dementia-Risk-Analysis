...
function fitSoftHyphenBreak(
  ...
  while (fitCount < graphemeWidths.length) {
    ...
    const nextLineWidth =
      fitCount + 1 < graphemeWidths.length
...
export function walkPreparedLines(prepared, maxWidth, onLine) {
  ...
  for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
    ...
    while (i < chunk.endSegmentIndex) {
      const kind = kinds[i];
      const w =
        kind === "tab" ? getTabAdvance(lineW, tabStopAdvance) : widths[i];
      if (kind === "soft-hyphen") {
        if (hasContent) {
      ...
      if (newW > maxWidth + lineFitEpsilon) {
        const currentBreakFitWidth =
          lineW + (kind === "tab" ? 0 : lineEndFitAdvances[i]);
        const currentBreakPaintWidth =
          lineW + (kind === "tab" ? w : lineEndPaintAdvances[i]);
        if (
        ...
        if (
          canBreakAfter(kind) &&
          currentBreakFitWidth <= maxWidth + lineFitEpsilon
        ) {
          appendWholeSegment(i, w);
...
export function layoutNextLineRange(prepared, start, maxWidth) {
  ...
  for (let i = normalized.cursor.segmentIndex; i < chunk.endSegmentIndex; i++) {
    ...
    const startGraphemeIndex =
      i === normalized.cursor.segmentIndex
        ? normalized.cursor.graphemeIndex
        : 0;
